...
class EthereumChain implements Chain {
    /**
     * Initialize EthereumChain with a 12 words seed phrase
     */
    public static fromMnemonic(mnemonic: string) {
        return new EthereumChain(ethereumPrivateKeyFromMnemonic(mnemonic));
    }
    ...
    /**
     * @param privateKeyOrMetaMask If a string is given LoomChain is initialized with the private key, otherwise it is initialized with the MetaMaskInpageProvider
     */
    ...
    public createERC20 = (asset: ERC20Asset) => {
        return new ERC20(asset.ethereumAddress.toLocalAddressString(), this.signer);
    };
