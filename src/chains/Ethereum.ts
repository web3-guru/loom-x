...
class Ethereum implements Chain {
    ...
    /**
     * Withdraw ETH from Gateway with `receipt` that was taken by calling `getPendingERC20WithdrawalReceipt()`.
     *
     * @link https://loomx.io/developers/en/transfer-gateway.html
     *
     * @param receipt
     */
    public withdrawETHWithReceiptAsync = (
        receipt: IWithdrawalReceipt
    ): Promise<ethers.providers.TransactionResponse | undefined> => {
        const gateway = this.getTransferGateway();
        if (receipt.tokenAmount) {
            const amount = toBigNumber(receipt.tokenAmount.toString());
            const signature = bytesToHexAddr(receipt.oracleSignature);
            return gateway.withdrawETH(amount, signature);
        } else {
    ...
    /**
     * Withdraw ERC20 of `assetAddress` from Gateway with `receipt` that was taken by calling `getPendingERC20WithdrawalReceipt()`.
     *
     * @link https://loomx.io/developers/en/transfer-gateway.html
     *
     * @param assetAddress
     * @param receipt
     */
    public withdrawERC20WithReceiptAsync = (
        assetAddress: Address,
        receipt: IWithdrawalReceipt
    ): Promise<ethers.providers.TransactionResponse | undefined> => {
        const gateway = this.getTransferGateway();
